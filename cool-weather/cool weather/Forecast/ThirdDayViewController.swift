        ...
        let dataTask =  session.dataTask(with: urlFutureWeather) { data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    if let url = URL(string: "https:\(weatherFutureResponse.forecast.forecastday[2].day.condition.icon)") {
                    ...
                    self.dates()
                        ...
                        self.dateLabel.text = "\(weatherFutureResponse.forecast.forecastday[2].date)"
                    self.conditionsLabel.text = "\(weatherFutureResponse.forecast.forecastday[2].day.condition.text)"
                    
                        //self.imageView.text =
                        
                    self.maxTempLabel.text = "Max temp \(weatherFutureResponse.forecast.forecastday[2].day.maxtemp_c)℃"
                    self.minTempLabel.text = "Min temp \(weatherFutureResponse.forecast.forecastday[2].day.mintemp_c)℃"
                    self.chanceOfRainLabel.text = "Chance of rain 🌧 is:  \(weatherFutureResponse.forecast.forecastday[0].day.daily_chance_of_rain)%"
                    self.windLabel.text = " Wind will be 💨: \(weatherFutureResponse.forecast.forecastday[2].day.maxwind_kph)k/ph"
                    self.humidityLabel.text = "Humidity level will be 💦: \(weatherFutureResponse.forecast.forecastday[2].day.avghumidity)%"
